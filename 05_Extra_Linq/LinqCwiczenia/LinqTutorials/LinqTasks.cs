...
namespace LinqTutorials
{
    public static class LinqTasks
    {
        ...
        {
            IEnumerable<Emp> result = Emps.Where(e => e.Salary == Emps.Max(e => e.Salary)).ToList();
            return result;
        }
        ...
        public static IEnumerable<object> Task11()
        {
            IEnumerable<object> result = Emps
                .GroupBy(e => e.Deptno)
                .Select(group => new { Deptno = group.Key, NumOfEmployees = group.Count() })
        ...
        public static IEnumerable<Dept> Task14()
        {
            ...
            IEnumerable<Dept> result = Depts.Where(d => empCounts.Any(ec => ec.Deptno == d.Deptno && (ec.Count == 5 || ec.Count == 0)))
                .OrderBy(d => d.Dname);
            //result =
            return result;
        }
        ...
        {
            IEnumerable<object> result = Emps
                // Odpowiednik WHERE Job LIKE '%A%' (wielkość liter zazwyczaj w SQL nie ma znaczenia, 
                // więc sprawdzamy i 'A' i 'a', lub robimy ToUpper())
                .Where(e => e.Job.ToLower().Contains("a")) 
                // Odpowiednik GROUP BY
                .GroupBy(e => e.Job)
                // Odpowiednik HAVING COUNT(*) > 2
                .Where(g => g.Count() > 2)
                // Odpowiednik ORDER BY COUNT(*) DESC
                .OrderByDescending(g => g.Count())
                // Odpowiednik SELECT Job AS Praca, COUNT(1) LiczbaPracownikow
    ...
    public static class CustomExtensionMethods
    {
        ...
        {
            var result = emps.Where(e => emps.Any(sub => sub.Mgr?.Empno == e.Empno))
                .OrderBy(e => e.Ename)
